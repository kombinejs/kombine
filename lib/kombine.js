...
function build(pagePath, options = {}) {
    if (pagePath) {
        if(!building){
            ...
            waitingList.push({pagePath: pagePath, options: options});
        }
    }
}
...
function buildNext(){
    ...
    if(waitingList.length > 0){
        let buildData = waitingList.shift();
        build(buildData.pagePath, buildData.options);
    }
}
...
function buildView(pagePath) {
    ...
    if (fs.existsSync(`${pagePath}/main.html`)) {
        ...
        for (var key in externalFile.importantCss) {
            importantCss += externalFile.importantCss[key] + '\n';
        }
        $('style').each(function () {
            if ($(this).attr('important')) {
                importantCss = $(this).html() + '\n' + importantCss;
            }
            else {
                css = $(this).html() + '\n' + css;
            }
...
function buildHtml(parentPath, html, componentList, templateHashList, externalFile) {
    ...
    $('style[src]').each(function () {
        ...
        if (fs.existsSync(`${filePath}`)) {
            ...
            if ($(this).attr('important'))
                externalFile.importantCss[hash] = cssContent;
            else
        ...
        $('template[src]').each(function () {
            ...
            $('style[src]').each(function () {
                ...
                if (!src.startsWith('/')) {
                    ...
                    cssFilePath = `${appRoot}${cssFilePath}`;
                }
